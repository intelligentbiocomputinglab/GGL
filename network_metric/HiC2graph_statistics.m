%%
clc;
el_input_path='~/network_metric/template_HiC/';
poincare_input_path='~/LG/';
statistic_output_path='~/network_metric/graph_statistics/';

filename='iced_BRD3187N_40000_chr1.mat';%normal colon
% filename='iced_BRD3187_40000_chr1.mat';%CRC
load([el_input_path,filename],'tri_hic');% load HiC data
chr_len=max([tri_hic(:,1);tri_hic(:,2)]);
el=tri_hic;
[hic0]=el2mat(el,chr_len);% triple HiC to matrix
...
writetable(tb,[poincare_input_path,'LG.hic_connection_prob.csv']);
...
for iter_num_max=1
    ...
    for prc_ind=1:length(prc_l)
        ...
        tmp=[id,hub_ranks];
        writematrix(tmp,strcat(statistic_output_path,replace(filename,'.matrix',''),'_CC_corr',num2str(iter_num_max),'_prc',num2str(prc),'.txt'));
        dist_mat=distances(G);
        dist_mat(isinf(dist_mat))=nan;
        tmp=mean(dist_mat,'all','omitnan');
        writematrix(tmp,strcat(statistic_output_path,replace(filename,'.matrix',''),'_ASPL_corr',num2str(iter_num_max),'_prc',num2str(prc),'.txt'));          
        toc;
    end
end
